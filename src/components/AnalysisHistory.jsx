...
import { Search, History, Eye, ArrowUpDown, ShieldAlert, CheckCircle, AlertCircle } from 'lucide-react'
...
export default function AnalysisHistory({ history, setActiveAnalysis }) {
  ...
  const filteredHistory = history.filter(item => {
    ...
    if (filterType === 'caution') return item.trustScore >= 40 && item.trustScore < 75;
  ...
  const getScoreStyle = (score) => {
    if (score >= 75) return 'text-cyber-success bg-cyber-success/10 border-cyber-success/20';
    if (score >= 40) return 'text-cyber-warning bg-cyber-warning/10 border-cyber-warning/20';
      ...
      <div className="glass-card rounded-3xl border border-slate-200/60 dark:border-cyber-border/50 overflow-hidden shadow-sm bg-white/40 dark:bg-cyber-card/55">
        ...
        {filteredHistory.length === 0 ? (
          <div className="p-16 text-center flex flex-col items-center justify-center">
            ...
            <h3 className="font-bold text-sm text-slate-700 dark:text-slate-300 uppercase tracking-wider">No Audits Found</h3>
            <p className="text-xs text-slate-400 max-w-sm mt-1.5 leading-relaxed font-semibold">
              Your query returned empty records. Clear the filter parameters or perform a new scan.
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-cyber-border/30 font-medium">
                {filteredHistory.map((item, idx) => (
                  <tr key={item.id} style={{ animationDelay: `${idx * 40}ms` }} className="animate-row-reveal hover:bg-slate-100/40 dark:hover:bg-cyber-card/85 transition-colors">
                    <td className="p-5 font-bold max-w-[200px] truncate text-slate-800 dark:text-slate-200">
                      {item.title}
                    </td>
                    <td className="p-5 text-slate-500 dark:text-slate-400 font-bold">{item.company}</td>
                    <td className="p-5 text-slate-400 truncate max-w-[150px] font-mono">{item.recruiterEmail}</td>
                    <td className="p-5 text-center">
                      <span className={`inline-block px-3 py-1 rounded-full border text-[10px] font-black ${getScoreStyle(item.trustScore)}`}>
                        {item.trustScore}/100
                      </span>
                    </td>
                    ...
                    <td className="p-5 text-slate-400 font-bold uppercase tracking-wider text-[10px]">{item.date}</td>
